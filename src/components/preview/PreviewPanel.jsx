import { useRef, useEffect, useState } from "react";
import { ResumeDocument } from "./ResumeDocument";
import { useResumeStore } from "@/store/resumeStore";
import { getPaperSize } from "@/lib/paperSizes";
import { FileText } from "lucide-react";

export function PreviewPanel() {
  const containerRef = useRef(null);
  const [scale, setScale] = useState(1);
  const { resume } = useResumeStore();
  const paper = getPaperSize(resume.meta.paperSize || "a4");

  useEffect(() => {
    function updateScale() {
      if (!containerRef.current) return;
      // 32px total horizontal padding (16px each side)
      const available = containerRef.current.clientWidth - 32;
      const ratio = available / paper.widthPx;
      setScale(Math.min(ratio, 1));
    }

    updateScale();
    const ro = new ResizeObserver(updateScale);
    if (containerRef.current) ro.observe(containerRef.current);
    return () => ro.disconnect();
  }, [paper.widthPx]);

  // Layout dimensions the flex container "sees"
  const scaledWidth  = Math.floor(paper.widthPx  * scale);
  const scaledHeight = Math.floor(paper.heightPx * scale);

  return (
    <div
      ref={containerRef}
      className="h-full overflow-y-auto bg-muted/40 flex flex-col items-center py-4 px-4"
    >
      {/* Paper size badge */}
      <div className="flex items-center gap-1.5 mb-3 text-xs text-muted-foreground">
        <FileText className="h-3.5 w-3.5" />
        <span>
          {paper.label} — {paper.widthMm} × {paper.heightMm} mm
        </span>
      </div>

      {/*
        Two-div scaling trick:
        • Outer div  → occupies the SCALED size in the flex layout (no overflow)
        • Inner div  → full document size, CSS-transformed to scale
        • transformOrigin "top left" is correct because inner is absolutely
          positioned at (0,0) inside the outer
      */}
      <div
        style={{
          width:    `${scaledWidth}px`,
          height:   `${scaledHeight}px`,
          position: "relative",
          flexShrink: 0,
        }}
      >
        <div
          style={{
            width:           `${paper.widthPx}px`,
            height:          `${paper.heightPx}px`,
            transformOrigin: "top left",
            transform:       `scale(${scale})`,
            position:        "absolute",
            top:  0,
            left: 0,
          }}
          className="shadow-xl ring-1 ring-black/10"
        >
          <ResumeDocument />
        </div>
      </div>

      {/* Bottom padding so content never hides behind the scroll-bar */}
      <div style={{ height: "24px", flexShrink: 0 }} />
    </div>
  );
}
